...
<div class="lg:basis-3/12 xl:basis-2/12 mb-4 lg:mb-0">
    <UserVertical userData="{userProfileData}" />
</div>
<div class="lg:lg:basis-9/12 xl:basis-10/12 mb-4 lg:mb-0">
    <h1 class="mb-2 text-2xl font-bold text-blue-700 tracking-tight leading-none xl:text-3xl dark:text-white">{userData.firstName} {userData.lastName}</h1>
    <p class="mb-2 font-semibold text-gray-800 lg:text-base xl:text-lg dark:text-gray-400">{userData.title}</p>

    {#if (userData.isTeachPhysically)}
    ...
    <div class="lg:flex lg:flex-row lg:justify-between mb-3 text-gray-500 text-sm mt-4">
        <p class="flex mb-1">
            ...
            <span>{userData.minimumPrice}<span class="text-xs">₺</span></span>
        </p>
        <p class="flex mb-1">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-1">
    ...
    <p class="text-sm text-justify leading-relaxed">{userData.about}</p>
</div>
