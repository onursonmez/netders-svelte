<script>
	...
	import SliderCard from '/src/components/SliderCard.svelte'
</script>
					...
					pagination: false,
						...
						<SplideSlide>
							<div class="flex flex-col gap-2 items-center w-full p-4 border border-blue-100 rounded-md">
								<SliderCard user={{...user, showRequest: true}} />
							</div>
						</SplideSlide>
						{/each}
...
<section class="dark:bg-gray-900 mt-4">
	<div>
		<div class="block p-6 rounded-lg shadow-md bg-white">
			<h2 class="mb-4 text-2xl font-bold tracking-tight leading-none lg:text-3xl dark:text-white">Okuma Yazma Özel Ders Verenler</h2>
			<div>
				{#await getUsers({'page': 1, 'pageSize': 12, 'keyword': 'okuma'})}
					Yükleniyor...
				{:then users}
					<Splide options={ {
					arrows: true,
