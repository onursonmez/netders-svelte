...
{#if data.shareModal}
<Modal on:close="{() => data.shareModal = !data.shareModal}">
	<div class="grow bg-white rounded-lg shadow-md">
		<div class="bg-[#fbfcff] border-b border-gray-100 p-6 rounded-t-lg text-lg font-semibold">Paylaş</div>
		<div class="p-6">
			<div class="flex flex-row justify-between">
				<div>
					<a href="https://twitter.com/intent/tweet?url={data.shareUrl}&text={data.shareText ?? data.title}">
				...
				<div>
					<a href="https://www.facebook.com/dialog/share?app_id=&href={data.shareUrl}&display=page&title={data.shareText ?? data.title}">
				...
				<div>
					<a href="https://www.linkedin.com/shareArticle?mini=true&url={data.shareUrl}&summary={data.shareText ?? data.title}">
				...
				<div>
					<a href="https://wa.me/?text={data.shareText ?? data.title} - {data.shareUrl}">
			...
			<hr class="my-4" />
			<div class="relative">
				<Clipboard text="{data.shareUrl}" let:copy on:copy={() => {Toastify({text: "Bağlantı kopyalandı 👍", className: "info", gravity: "bottom"}).showToast()}}>
				...
				<div class="mt-2"><input type="text" value="{data.shareUrl}" class="w-full rounded-md border border-gray-300 hover:border-gray-400 focus:border-blue-600 focus:ring-0 text-gray-500" /></div>
			</div>
		</div>
	</div>
</Modal>
...
<a href="{ data.cardLink }" target="_blank" rel="noreferrer">
	<img class="rounded-full mx-auto w-48 h-48" src="{data.photoUrl}" alt="">
</a>
<div class="flex flex-col w-full justify-between pl-4 leading-normal mt-2">
	{#if data.title}
	<a href="{ data.cardLink }" target="_blank" rel="noreferrer" class="block lg:hidden">
		<h5 class="mb-2 text-xl font-bold tracking-tight text-blue-700 text-center">{ data.title }</h5>
	</a>
	{/if}

	{#if data.subTitle}
	<p class="mb-2 font-semibold text-gray-800 lg:text-base xl:text-lg dark:text-gray-400 text-center block lg:hidden">{data.subTitle}</p>
	{/if}

	{#if (data.price || data.locationName || data.totalComment) }
	<div class="flex flex-row gap-2 justify-between text-gray-500 text-sm mt-1 block lg:hidden">
		{#if data.price}
		<div>
		...
		<div>
			...
			{ data.totalComment ?? 0 } yorum
		</div>
	</div>
	{/if}

	{#if data.description}
		<p class="text-sm text-justify leading-relaxed mt-4 block lg:hidden">{data.description}</p>
	{/if}

	{#if (data.showApprovedBadge || data.createdAt || data.showIsOnlineBadge || data.shareUrl) }
	<div class="flex items-center justify-center gap-2 mt-4">
		{#if data.showApprovedBadge}
		...
		{#if data.createdAt}
			...
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{dayjs(new Date(data.createdAt.date)).fromNow()} üye oldu</Tooltip>
		{/if}

		{#if data.showIsOnlineBadge}
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 outline-none" class:text-gray-300={!data.isOnline}>
				<path stroke-linecap="round" stroke-linejoin="round" d="M5.636 5.636a9 9 0 1012.728 0M12 3v9" />
			</svg>
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{data.isOnline ? 'Çevrimiçi' : 'Çevrimdışı'}</Tooltip>
		{/if}

		{#if data.shareUrl}
			<button on:click={() => data.shareModal = !data.shareModal}>
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 outline-none">
	...
	{#if data.requestButtonUrl}
	<a href="{data.requestButtonUrl}" class="bg-blue-700 p-2 rounded-full justify-center text-sm flex text-white mt-4">
