<script>
	...
	let shareUrl = import.meta.env.VITE_BASE_URL + '/' + user.username

</script>

{#if shareModal}
<Modal on:close="{() => shareModal = !shareModal}">
	<div class="grow bg-white rounded-lg shadow-md">
		...
		<div class="p-6">
			<div class="flex flex-row justify-between">
				<div>
					<a href="https://twitter.com/intent/tweet?url={shareUrl}&text={user.firstName} {user.lastName}">
				...
				<div>
					<a href="https://www.facebook.com/dialog/share?app_id=&href={shareUrl}&display=page&title={user.firstName} {user.lastName}">
				...
				<div>
					<a href="https://www.linkedin.com/shareArticle?mini=true&url={shareUrl}&summary={user.firstName} {user.lastName}">
				...
				<div>
					<a href="https://wa.me/?text={user.firstName} {user.lastName} - {shareUrl}">
			...
			<hr class="my-4" />
			<div class="relative">
				<Clipboard text="{shareUrl}" let:copy on:copy={() => {Toastify({text: "Bağlantı kopyalandı 👍", className: "info", gravity: "bottom"}).showToast()}}>
				...
				<div class="mt-2"><input type="text" value="{shareUrl}" class="w-full rounded-md border border-gray-300 hover:border-gray-400 focus:border-blue-600 focus:ring-0 text-gray-500" /></div>
			</div>
		</div>
	</div>
</Modal>
...
<div class="flex flex-col w-full justify-between pl-4 leading-normal mt-2">
	...
	<p class="mb-2 font-semibold text-gray-800 lg:text-base xl:text-lg dark:text-gray-400 text-center block lg:hidden">{user.title}</p>
	...
	<div class="flex flex-row gap-2 justify-between text-gray-500 text-sm mt-1 block lg:hidden">
		...
		<div>
			...
			{#if user.county?.id}
				{user.county.title}, {user.city.title}
			{:else}
				{user.country.title}
			{/if}
		</div>
	...
	<div class="flex items-center justify-center gap-2 mt-4">
		...
		{#if user.createdAt}
			...
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{dayjs(new Date(user.createdAt.date)).fromNow()} üye oldu</Tooltip>
		{/if}

		{#if user.showIsOnlineBadge}
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 outline-none" class:text-gray-300={!user.isOnline}>
				<path stroke-linecap="round" stroke-linejoin="round" d="M5.636 5.636a9 9 0 1012.728 0M12 3v9" />
			</svg>
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{user.isOnline ? 'Çevrimiçi' : 'Çevrimdışı'}</Tooltip>
		{/if}
	...
	{#if user.showRequest}
	<a href="/ozel-ders-talebi-olustur/{user.username}" class="bg-blue-700 p-2 rounded-full justify-center text-sm flex text-white mt-4">
