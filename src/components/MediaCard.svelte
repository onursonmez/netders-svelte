<script>
	...
	import { getUserPhotoFunction } from '/src/functions/userPhotoFunction'
...
{#if pageData.shareModal}
	<Modal on:close="{() => pageData.shareModal = !pageData.shareModal}">
		<h3 slot="header" class="text-xl font-medium text-gray-900 dark:text-white pb-4">Paylaş</h3>
		<div class="flex flex-row justify-between">
			<div>
				<a href="https://twitter.com/intent/tweet?url={pageData.shareUrl}&text={pageData.shareText ?? pageData.title}">
			...
			<div>
				<a href="https://www.facebook.com/dialog/share?app_id=&href={pageData.shareUrl}&display=page&title={pageData.shareText ?? pageData.title}">
			...
			<div>
				<a href="https://www.linkedin.com/shareArticle?mini=true&url={pageData.shareUrl}&summary={pageData.shareText ?? pageData.title}">
			...
			<div>
				<a href="https://wa.me/?text={pageData.shareText ?? pageData.title} - {pageData.shareUrl}">
		...
		<hr class="my-4" />
		<div class="relative">
			<Clipboard text="{pageData.shareUrl}" let:copy on:copy={() => {Toastify({text: "Bağlantı kopyalandı 👍", className: "info", gravity: "bottom"}).showToast()}}>
			...
			<div class="mt-2"><input type="text" value="{pageData.shareUrl}" class="w-full rounded-md border border-gray-300 hover:border-gray-400 focus:border-blue-600 focus:ring-0 text-gray-500" /></div>
		</div>
	</Modal>
{/if}
...
<div class="flex flex-col w-full justify-between pl-4 leading-normal mt-2">
	...
	<div class="flex items-center justify-center gap-2 mt-4">
		...
		{#if pageData.createdAt}
			...
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{dayjs(new Date(pageData.createdAt.date)).fromNow()} üye oldu</Tooltip>
		{/if}

		{#if pageData.showIsOnlineBadge}
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 outline-none" class:text-gray-300={!pageData.isOnline}>
				<path stroke-linecap="round" stroke-linejoin="round" d="M5.636 5.636a9 9 0 1012.728 0M12 3v9" />
			</svg>
			<Tooltip style="custom" class="text-xs bg-blue-700 border-blue-700 text-white">{pageData.isOnline ? 'Çevrimiçi' : 'Çevrimdışı'}</Tooltip>
		{/if}

		{#if pageData.shareUrl}
			<button on:click={() => pageData.shareModal = !pageData.shareModal}>
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 outline-none">
	...
	{#if pageData.requestButtonUrl}
	<a href="{pageData.requestButtonUrl}" class="bg-blue-700 p-2 rounded-full justify-center text-sm flex text-white mt-4">
