...
<div class="lg:lg:basis-9/12 xl:basis-10/12 hidden lg:block">
    ...
    <p class="mb-2 font-semibold text-gray-800 lg:text-base xl:text-lg dark:text-gray-400">{pageData.subTitle}</p>

    {#if (pageData.isTeachPhysically)}
    ...
    {#if (pageData.price || pageData.totalComment || pageData.locationName) }
    <div class="lg:flex lg:flex-row lg:justify-between mb-3 text-gray-500 text-sm mt-4">
        ...
        <p class="flex mb-1">
            ...
            <span>{pageData.price}<span class="text-xs">₺</span></span>
        </p>
        {/if}
        {#if pageData.totalComment }
        <p class="flex mb-1">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-1">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
            </svg>
            {pageData.totalComment} yorum
        </p>
        {/if}
    ...
    <p class="text-sm text-justify leading-relaxed">{pageData.description}</p>
</div>
